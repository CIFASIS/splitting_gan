...
def extensible_param(name, *args, **kwargs):
    if name not in _params:
        _params_extensible.append(name)
    return param(name, *args, **kwargs)

def get_extensible_params():
    return [(n, _params[n]) for n in _params_extensible]
